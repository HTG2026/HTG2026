...
- `npm run check:vercel` – **Ping production** (pass your URL, or set `NEXT_PUBLIC_SITE_URL` in `.env.local`). On Vercel, the app uses automatic `VERCEL_URL` for canonical links if unset — see [docs/VERIFY_VERCEL_CONNECTION.md](docs/VERIFY_VERCEL_CONNECTION.md)
